...
cumulative <- function(towers, plot_df, plot_data) {
  cumulative_plot <- ggplot() +
    stat_ecdf(data = towers, aes_string(x = plot_data), color = 'red', geom = "line") +
    stat_ecdf(data = plot_df, aes_string(x = plot_data), color = 'blue', geom = "line")
...
# LST Daytime
cumulative_daytime <- cumulative(towers_coords_df, lst_df, "lst_day")
cumulative_daytime
